...
class ComplaintForm(FlaskForm):
    ...
    options = SelectField("Select a student resource to assist with your issue.", choices=[("", "Select an option..."), ("VCampus", "VCampus"), ("OSIS", "OSIS"), ("VCampus & OSIS", "VCampus & OSIS")], validators=[DataRequired()],)
...
class ApplicantForm(FlaskForm):
    ...
    nationality = SelectField(
        "Nationality", choices=[("Ghana", "Ghana"), ("Nigeria", "Nigeria")]
    )
    ...
    entrymode = SelectField(
        "Entry Mode", choices=[("Direct", "Direct"), ("Cohort", "Cohort")]
    )
...
class ApplicantExams(FlaskForm):
    examtype = SelectField(
        "Exam Type", choices=[("Wassce", "Wassce"), ("Nov-Dec", "Nov-Dec")]
    )
...
class ApplicantExamresult(FlaskForm):
    applicantexam = SelectField(
        "Applicant Exam", choices=[("Degree", "Degree"), ("Diploma", "Diploma")]
    )
    subject = SelectField(
        "Subject",
        choices=[
            ("Dip.Enterprise Management", "Dip.Enterprise Management"),
            ("Dip.Enterprise Management", "Dip.Enterprise Management"),
        ],
    )
    grade = SelectField(
        "Grade",
        choices=[
            ("1st Class Upper", "1st Class Upper"),
...
class ApplicantContant(FlaskForm):
    contactrelationship = SelectField(
        "Relationship", choices=[("Mother", "Mother"), ("Father", "Father")]
    )
...
class ApplicantIdentityForm(FlaskForm):
    identitytype = SelectField(
        "Type", choices=[("Ghana Card", "Ghana Card"), ("Passport", "Passport")]
    )
...
class ApplicantAnswer(FlaskForm):
    applicantanswer = StringField(
        "How did you get to know about Central University", validators=[DataRequired()]
    )
    submit = SubmitField("Next")
...
class ApplicantHall(FlaskForm):
    applicanthall = SelectField(
        ...
        choices=[
            ("CU Male Hostel", "CU Male Hostel"),
            ("CU Female Hostel", "CU Female Hostel"),
        ],
    )
    submit = SubmitField("Next")
